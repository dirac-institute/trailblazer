...
def get_secrets_filepath():
    ...
    if CONF_FILE_ENVVAR in os.environ:
        filePath = os.environ[CONF_FILE_ENVVAR]
    else:
        filePath = CONF_FILE_PATH

    return URI(filePath).ospath
...
def yaml_to_dict(filePath):
    ...
    """
    with URI(filePath).open() as stream:
...
class Config():
    ...
    def expandFromOrigin(self, path):
        """Resolves the absolute path as if path was relative to `origin`.
        If the given path was already absolute, returns the path unchanged.
...
class SecretsConfig(Config):
    ...
    defaults = {
        'db': {
            ...
            "name": str(Path(__file__).resolve().parent.parent.joinpath("db.sqlite3"))
            },
        "secret_key": "alalala",
    }
    """Default instantiation values."""
